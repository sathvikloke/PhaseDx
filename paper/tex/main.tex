...
primary cohort. Figure~\ref{fig:input} shows what the estimators were given on
that cohort --- the magnitude channel, the two phase channels and the raw angle
they are computed from, for one slice with an annotated tumour and one without,
read out of the cache in stored order. It is there so that a reader can see the
input rather than infer it from a channel name; it illustrates and does not
...
against the same axis and the same pre-registered ceiling in Supplementary
...
The worked-example cohorts are NYU fastMRI releases and are \emph{not}
redistributed with this work. They are obtained by application to the NYU
fastMRI Initiative at \url{https://fastmri.med.nyu.edu} under its data sharing
agreement \citep{knoll2020fastmri,zbontar2018fastmri,tibrewala2023fastmriprostate,solomon2025fastmribreast}.
That agreement forbids onward distribution of the dataset or of files derived
from it, so no reconstructed image, cache file or slice is included in the
released repository; the figures of those cohorts in this article and its
supplement are supplied to the journal directly, and the code that regenerates
them from a reader's own copy of the data is released.
...
\caption{\textbf{The estimator's input on the pre-specified primary cohort
