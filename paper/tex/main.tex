...
%  booktabs, siunitx, caption, natbib, microtype, hyperref only.
%
...
%    jarkman2022generalization, kapoor2023leakage, knoll2020fastmri,
%    maierhein2018rankings,
...
%    setio2017luna16, solomon2025fastmribreast, tampu2022leakage,
%    tibrewala2023fastmriprostate,
...
%    zbontar2018fastmri, zech2018pneumonia, zhao2022fastmriplus
%  (osciiart2020baseline is a Kaggle notebook; rempe2024kspace and
%   zbontar2018fastmri are arXiv preprints -- all must be cited as such.)
%
%  ATTRIBUTION THAT IS NOT OPTIONAL: knoll2020fastmri and zbontar2018fastmri
%  are required by the NYU fastMRI Data Sharing Agreement, paragraph 6(b).
%  The agreement also requires the literal string "NYU fastMRI" in the
%  abstract (para 6a) and the "Data used in the preparation of this article
%  were obtained from the NYU fastMRI Initiative database" paragraph in the
%  Methods (para 6c). Both are present below. Removing any of the three puts
%  the manuscript in breach; see refs.bib section 10.
% =====================================================================
...
% microtype does the character protrusion and font expansion that stops
% justified text opening up rivers of white space. Stock TeX Live, already
% used by supplement.tex. It also removes most overfull boxes on its own.
...
% ---------------------------------------------------------------------
%  TITLE PAGE
%  Hand-set rather than \maketitle. \maketitle centres the author list and
%  drops a \thanks footnote at the foot of the page, which separates an
%  author from their affiliation by the height of the page. Journal style
%  is superscript markers against a numbered affiliation list, so that is
%  what this builds. Nothing here is a float, so nothing can migrate.
% ---------------------------------------------------------------------
...
\begin{center}
{\LARGE\bfseries What a slice-level benchmark certifies\\ without the pixels\par}
\vspace{0.55em}
{\large\bfseries A label-file audit of seven public benchmarks, and a
protocol-frozen\\ random-sample screen of how often the check is reported\par}

\vspace{1.6em}

{\large Sathvik Loke\affmark{1,*}\quad Ethan Johnson\affmark{2}\quad
Neeraj Movva\affmark{1}\quad Aditya Raut\affmark{3}\par}
\end{center}
...
\begin{tabbing}
\hspace{1.3em}\=\kill
\affmark{1}\>Illinois Mathematics and Science Academy, Aurora, Illinois\\
\affmark{2}\>Central High School, Springfield, Missouri\\
\affmark{3}\>Neuqua Valley High School, Naperville, Illinois\\[0.5em]
\affmark{*}\>Corresponding author: Sathvik Loke, \texttt{sloke@imsa.edu}
\end{tabbing}

\vspace{-0.4em}
\begin{tabular}{@{}p{0.16\textwidth}@{}p{0.82\textwidth}@{}}
\textbf{Co-authors} & \texttt{ethanthekek21@gmail.com}, \texttt{nmovva@imsa.edu},
\texttt{aditya.sraut09@gmail.com}\\[0.35em]
\textbf{Funding} & The authors received no funding for this work.\\[0.35em]
\textbf{Conflicts} & All authors declare no conflicts of interest.\\[0.35em]
\textbf{Data} & NYU fastMRI, under its Data Sharing Agreement; all other
sources are public. See Data and Code Availability.\\
\end{tabular}
}
...
%
% AND DO NOT CUT THE STRING "NYU fastMRI" OUT OF THE ABSTRACT. It is in the
% Materials and Methods paragraph below, and it is there because the fastMRI
% Data Sharing Agreement, paragraph 6(a), requires it verbatim: "In any
% published abstract, I will cite 'NYU fastMRI' as the source of the data in
% the abstract." It is two words. Cut something else.
\section*{Abstract}
...
rank-inversion analysis on 21 method configurations across five NYU fastMRI
cohorts compared
between-unit rank disagreement against within-unit resampling noise. A
protocol-frozen random-sample screen of a frozen \num{9979}-record PubMed frame
...
half the reported margin over chance, and in a protocol-frozen sample of the
...
0.469 (IQR: 0.437, 0.490; $n=9$) of the published margin over chance; on one of
the nine, LUNA16, it reached none of it ($-0.002$).
\item In a protocol-frozen random sample, 0 of 91 readable papers reported any
...
obtained in a protocol-frozen random sample of this literature reported such a
...
(October 2020), bins the label rate against relative slice position,
$P(\text{label}\mid z)$, on the
training set and applies it to the test set with no pixels, scoring 0.33 on the
public leaderboard against 0.44 for a constant predictor
\citep{osciiart2020baseline}; that is the same construction, four years
earlier. Yan et al., in the paper that
...
the benchmarks on which nothing is reachable; (c) a protocol-frozen estimate of
...
Institutional review board approval was not sought and, in the authors'
assessment, was not required: the work is secondary analysis of publicly
released, de-identified datasets obtained under their respective data use
agreements, together with a review of published literature and public regulatory
summaries. It is not human-subjects research as defined at 45 CFR 46.102(e),
because the authors obtained no data through intervention or interaction with any
living individual and no identifiable private information. The imaging data are
governed by the data use agreements cited in the data-source paragraph below
rather than by an approval issued to this study. The audit analyses published label files only; no pixel data
were downloaded. The screening protocol, codebook, sampling frame and seeded sample
were committed to a public repository before any analysis-sample record was coded
(commit \texttt{a64d202}). \textbf{The screen was not deposited with
OSF Registries, PROSPERO or any other registry in advance and is therefore not
prospectively registered}, a reportable deviation from our own protocol
...
% -------------------------------------------------------------------------
% DO NOT DELETE, ABRIDGE OR MOVE THE FOLLOWING PARAGRAPH.
% It is the acknowledgement mandated by the NYU fastMRI Data Sharing
% Agreement, paragraph 6(c), which closes: "further acknowledge that
% inclusion of some variation of the language shown above is mandatory."
% All five worked-example cohorts are NYU fastMRI releases.
% -------------------------------------------------------------------------
...
\url{https://fastmri.med.nyu.edu}. The primary goal of fastMRI is to test whether
machine learning can aid in the reconstruction of medical images. The five
cohorts used here are the fastMRI knee and brain raw $k$-space releases
\citep{knoll2020fastmri,zbontar2018fastmri}, fastMRI Prostate --- T2 and DWI
\citep{tibrewala2023fastmriprostate} --- and fastMRI Breast
\citep{solomon2025fastmribreast}.
...
with each subject tested exactly once.

\textbf{What the input actually is.} The magnitude condition is fed as a single
per-slice z-scored magnitude channel, so that global slice brightness --- which
tracks receive gain, and therefore scanner and session --- is not available to it.
The phase
condition is fed as two channels, $\sin\varphi$ and $\cos\varphi$, and never as
raw radians: phase wraps, so $-\pi$ and $+\pi$ are the same angle but maximally
distant as numbers, and a convolution over wrapped phase sees a false edge at
every wrap. $\sin\varphi$ and $\cos\varphi$ are already bounded in $[-1,1]$ and
are deliberately not rescaled per slice, which would destroy their physical
units. The \emph{both} condition is the concatenation of the three. All three of
these channels are shown for the primary cohort in the Results, beside the raw
angle they are computed from, which no estimator ever receives. Supplementary
Figure S3 shows the remaining four cohorts more briefly: the magnitude channel
and that same raw angle, not $\sin\varphi$ and $\cos\varphi$.

Reconstructions were validated against the
...
than a ground truth. Because two images of the same body part correlate even when
they are different slices, each correlation is also reported against an
anatomy-support floor, per cohort and per slice, in Supplementary Figure S2 and
Supplementary Section S4.

\subsection{The protocol-frozen prevalence screen}
...
into the pre-specified denominator.
...
The audit tool is \texttt{trivialbaselines} v1.0 (CC BY 4.0)
...
evaluation unit, the zero-image positional baseline reaches slice AUROC 0.851
(95\% CI: 0.816, 0.887) against a published headline of 0.861, for a trivial
fraction of 0.973 (95\% CI: 0.876, 1.073). That is the diffusion arm, which is
the arm those authors state they use (``we only work on the DWI data''); the
T2 arm of the same release, which they do not use, gives 0.854 (95\% CI: 0.812,
0.891) and a fraction of 0.981 (95\% CI: 0.865, 1.084), and we report it because
their stated cohort size of 9{,}508 labelled slices is the row count of the
release's T2 label file rather than of the diffusion file they describe using
(9{,}490). The result does not depend on fitting anything: the diffusion bin
sweep gives 0.834 / 0.842 / 0.851 / 0.852 / 0.841 over 5 / 10 / 20 / 30 / 50
bins, and the no-fit centrality score reaches 0.841 (DWI) and 0.825 (T2). \textbf{The general claim that trivial baselines match published
...
says nothing about whether anyone checks; the protocol-frozen screen answers that
...
set.} Ninety-one were included and readable, against a pre-specified target of
75.
...
and the protocol's pre-specified remedy was triggered. The diagnosis does not excuse the
...
under nine falsification criteria in the released scoring script
(\texttt{pipeline/s06\_report.py}). \textbf{Unlike the screening protocol, these
criteria were not committed before the runs they judge}, and the set was amended as
the study ran, every change adding a requirement; prostate T2, the primary cohort,
was \textbf{NOT SUPPORTED} under both the seven- and the nine-criterion scoring
(Supplementary Section~S4).
Figure~\ref{fig:input} shows what the estimators were given on that cohort, read
out of the cache in stored order; it illustrates and does not
adjudicate, and none of the results below rest on it. All three clinical cohorts
were \textbf{NOT SUPPORTED} at the
...
CI: 0.870, 0.966) on 136 independent test subjects. Stratifying the comparison
within site does not remove it, but the within-site number rests on one of the
two sites and must be read that way: the cohort's 136 test subjects split 98/38
between them, and only the 98-subject site clears the 10-per-class floor fixed
before the analysis, at 0.979 (95\% CI: 0.953, 0.996) on the first of two seeds
and 0.974 (95\% CI: 0.940, 0.997) on the second. The 38-subject site, which
carries only 6 subjects in one class, sits at 0.542 and 0.578 on the same two
seeds and is below that floor. What the separability analysis licenses is
therefore the qualitative statement --- the effect survives conditioning on site
in the only site where it can be tested --- and not a within-site point estimate
for the cohort. It is not
separable from scanner model or coil array, so we may say phase encodes hardware
and may not decompose that hardware further. Every confound control written for
the three clinical cohorts, including the ones that land near chance, and every
site stratum including the one below the floor, is plotted
against the same axis and the same C6 ceiling in Supplementary
Figure S1. \emph{No aggregation rescues the
...
positional predictor in 2020 \citep{osciiart2020baseline} and the paper that defines the
DeepLesion lesion-type task published a location-only baseline in 2018
\citep{yan2018deeplesiongraphs}. What is new
...
--- no \texttt{not\_assessable} level --- and the pre-specified remedy was
...
hypothesis, and a fifth block of records lies outside the pre-specified stopping
...
reachable with no pixels is 0.469 (IQR: 0.437, 0.490; $n=9$), and on one of them,
LUNA16, none of it is reachable. Whether the unit also changes \emph{which
method wins} we could not establish, and neither can any published two-unit
comparison we located. In a protocol-frozen random sample of the literature that
...
The audit tool (\texttt{trivialbaselines} v1.0, CC BY 4.0), the per-benchmark
JSON payloads and cards, the label-table preparation scripts, the independent
reimplementation of the RSNA ICH unit-collapse computation, the trivial-fraction
assembler, and the full prevalence-screen package --- frozen protocol and codebook
with changelogs, sampling frame and seeded permutation with their SHA-256 digests,
the four sealed screener files, the adjudication audit trail, the re-code overlay,
the reserve blocks and the pooling script --- are released at
\url{https://github.com/sathvikloke/PhaseDx} under a CC~BY~4.0 licence and
archived at \url{https://doi.org/10.5281/zenodo.21814952}. That is the concept
DOI, which always resolves to the most recent version; the version archived at
submission is \texttt{10.5281/zenodo.21814953}. The 44 unreachable records are
listed by PMID so a reader with better access can complete the screen.

The worked-example cohorts are NYU fastMRI releases, obtained by application to the
NYU fastMRI Initiative at \url{https://fastmri.med.nyu.edu} under its data sharing
agreement \citep{knoll2020fastmri,zbontar2018fastmri,tibrewala2023fastmriprostate,solomon2025fastmribreast},
which forbids onward distribution of the dataset or of derived files. No
reconstructed image, cache file or slice is in the released repository; figures are
supplied to the journal directly and the code that regenerates them from a reader's
own copy is released.
...
\begin{table}[htbp]
\centering
\caption{Protocol-frozen prevalence screen: flow and endpoints. 250 papers sampled
from a frozen \num{9979}-record PubMed frame at permutation positions 1--100 and
111--260, the second range drawn under the pre-specified extension rule, which
...
is never pooled into the pre-specified denominator.
\end{table}
...
4 & Report the zero-image baselines beside every headline number & 0.851 against a published 0.861, with no pixels, on the diffusion arm those authors state they use (preprint comparator)\\
...
\caption{\textbf{Protocol-frozen prevalence screen: flow and censoring.}
...
screened block against the pre-specified 15\% threshold: the rate does not fall
...
and is never pooled into the pre-specified denominator.}
\label{fig:prisma}
\end{figure}
...
NOT SUPPORTED against the nine falsification criteria. \textbf{(B)} The background-only
...
% ---------------------------------------------------------------------
% NOTE: figures/fig6_qualitative_phase.pdf is NOT in the public git
% repository. It embeds fastMRI-derived MRI slices, and the NYU fastMRI
% data sharing agreement permits their use in an academic publication but
% forbids redistribution. It is named in paper/tex/.gitignore and must be
% supplied to the journal out of band; make_figures.py rebuilds it from
% pipeline_out/cache/prostate_t2.h5 in one command. See paper/tex/README.md.
% ---------------------------------------------------------------------
\begin{figure}[htbp]
\centering
% This figure contains fastMRI image data and is therefore NOT in the public
% git repository -- see the note above. \IfFileExists keeps the document
% building for anyone who clones the repo without it, instead of failing with
% "file not found" on a figure they were never permitted to receive.
\IfFileExists{figures/fig6_qualitative_phase.pdf}{%
  \includegraphics[width=\textwidth]{figures/fig6_qualitative_phase.pdf}%
}{%
  \fbox{\parbox{0.92\textwidth}{\vspace{1em}\centering\small
  \textbf{Figure omitted from the public repository.}\\[0.4em]
  This panel displays NYU fastMRI image data. The Data Sharing Agreement
  permits it in an academic publication but not in a redistributable archive,
  so the file is excluded from version control and supplied to the journal
  separately. Rebuild it from the licensed cache with
  \texttt{python paper/tex/make\_figures.py}.\vspace{1em}}}%
}
\caption{\textbf{The estimator's input on the primary cohort
(fastMRI Prostate, T2), for one slice of each class.} Left to right: the
magnitude channel as fed, per-slice z-scored; phase channel 1, $\sin\varphi$;
phase channel 2, $\cos\varphi$; and, for orientation only, the raw phase in
radians on a cyclic scale so that the $\pm\pi$ wrap reads as a wrap. Columns 1--3
are the arrays the network receives; column 4 is never fed to it. The cyan
outline is the stage-2 body mask, whose complement defines the background-only
control. \textbf{(A)} A slice with a tumour annotated on it: cache row 1312,
\texttt{file\_prostate\_AXT2\_052.h5}, slice 9, patient 52, label 1.
\textbf{(B)} A slice with no tumour annotated: cache row 1224,
\texttt{file\_prostate\_AXT2\_049.h5}, slice 11, patient 49, label 0. Both are
drawn from the official test split by a rule fixed for every panel in this
manuscript --- the median row at that label, in cache order --- and neither was
chosen by any outcome. The arrays are read from the stage-2 cache in stored
order, with no transpose, no re-reconstruction and no per-slice rescaling of
phase; the magnitude \emph{display} is windowed at $\pm$ the 99th centile of
$|z|$, as its colour bar states, and the three other columns are shown over
their full range. \textbf{This figure illustrates the input; it is not evidence for or
against the phase hypothesis.} That question is settled by the intervals in
Figure~\ref{fig:case} and by the falsification suite, not by how a slice looks.
Data used in the preparation of this figure were obtained from the NYU fastMRI
Initiative database \citep{knoll2020fastmri,zbontar2018fastmri,tibrewala2023fastmriprostate}.}
\label{fig:input}
\end{figure}
