...
We are disclosing a prior posting that no longer resolves, because the policy
asks for earlier versions and because a withdrawn record is still discoverable.
On 2026-08-05 we deposited a research compendium on Zenodo containing analysis
code, per-benchmark outputs and the source of a longer manuscript that includes
some of the results reported here. We withdrew it on 2026-08-14. Both DOIs,
\texttt{10.5281/zenodo.21814952} and \texttt{10.5281/zenodo.21814953}, now return
HTTP 410; the DataCite metadata record for the first remains publicly findable,
which is why we would rather name it than have you find it. We can supply the
withdrawn contents, in full or in part, on request.

The code and outputs are instead maintained in the public repository cited in the
data sharing statement on the title page, which carries no manuscript text.

Because the policy asks how the submitted manuscript differs from any earlier
version, here is the map rather than a characterisation of it:
...
\item \textbf{In the withdrawn deposit, deliberately not here.} A k-space phase
study and the other components of the larger project, one of which is being
substantially reworked. No claim in this manuscript rests on any of them.
\end{itemize}
...
mirror. We have since obtained the official label file and reconciled the mirror
against it row for row: across all six labels on every one of the 752,802
images the mirror holds, there is no disagreement, and it contains no image the
official release does not. The one image it lacks is the one an internal
gapless-counter test predicted before that file was seen. What that comparison
cannot establish is the mirror's series and patient groupings or its slice order,
because the official file carries neither a subject identifier nor a slice index,
which is why the mirror is needed; those still rest on the tests in Table 2. We
would rather state the residual limit plainly than have a reviewer find it.
...
than the usual disclosure. Every number in the Results is a deterministic output
of released code run on a published label file, and no reported value was
generated by a language model. Large language model assistance was used in
drafting and editing this manuscript and in implementing that analysis code; the
tool was \textbf{Claude (Opus~5), Anthropic}, accessed 2026-07-27 to
2026-08-14.
