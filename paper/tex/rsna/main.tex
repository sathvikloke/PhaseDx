...
as control, were fit on training slices of subject-disjoint splits
...
alongside. Splits were subject-disjoint throughout: a benchmark's own published
train/test assignment where one exists, otherwise a five-fold split scored out
of fold and pooled, so every subject is tested exactly once. Confidence
intervals (CIs) are percentile bootstrap over \emph{subjects}---a subject drawn
twice contributes all of that subject's slices twice---with 2000 replicates for
the primary hemorrhage analysis and between 200 and 2000 for the remaining runs,
as each run's own output records. The naive
...
subjects, with the replicate count and seed recorded for each run and degenerate
replicates counted rather than dropped. Analyses used Python 3.11 (Python Software
...
same divergence and two do not (Table 3, Fig 2). DeepLesion does not collapse
...
than as a count of verdicts (Table 4, Fig 3).
...
to 0.003 AUC at both units. Readers are directed to the released code rather
...
0.613. LUNA16's $-0.002$ is computed on sensitivity at one false positive per
scan, one operating point of the challenge's own competition performance metric,
against a random-score reference of 0.0027 rather than against 0.5. No
