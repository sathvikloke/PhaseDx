...
%  depend on main.tex, and it loads no external graphics or .bib file.
...
(\mbox{S1--S7}) is referenced from the main text but does not depend on it.
...
\texttt{trivialbaselines} v1.0 (MIT), which depends on \texttt{numpy} and
...
HDF5 file, by Pearson correlation recomputed for every cached slice.
...
Two of 2{,}039 slices fall below 0.99 and none below 0.95. This cohort was fixed as
the pre-specified primary cohort of the case study on two grounds decided in advance
--- it is the largest (67 patients) and it is the only one whose reconstruction is
validated against an unregularised vendor reference.
...
States patient-level train/test disjointness & \textbf{1 / 30} \\
...
903 devices coded on closely related fields has already been published.
...
disjointness is stated in one Summary out of thirty. That is a reporting observation
...
\caption{PRISMA 2020 checklist, adapted. \emph{Eight items are marked not done.}}
\label{tab:prisma}\\
\toprule
\# & item & status & notes \\
\midrule
...
24a & Registration name, registry and number, or a statement that the review was not registered & \textbf{not done in the manuscript; the statement is drafted} & \textbf{The review was not registered.} No OSF, PROSPERO or other deposit was made before screening. The protocol, codebook, frame, permutation and sample were committed to a public repository before any analysis-sample record was coded, which is a weaker and different guarantee. See Section~S7. \\
...
25 & Sources of financial and non-financial support & \textbf{not done} & No funding statement has been written. \\
26 & Competing interests & \textbf{not done} & No competing-interests statement has been written, and none has been collected from the co-authors. Each author must declare individually. \\
27 & Availability of data, code and materials & addressed, with one gap & The frozen protocol and codebook, the frame with its digest, the seeded permutation, the sealed screener files with every quote and search string, the analysis scripts and outputs, and the released tool are all available; the unreachable PMIDs are listed so a reader with better access can finish the screen. \textbf{Gap:} the archival deposit has metadata prepared but no DOI minted, so every DOI reference is currently a placeholder. Retrieved full texts are not redistributable; URLs plus verbatim quotes are the audit trail instead. \\
\end{longtable}}
...
status & items \\
\midrule
addressed & 3, 4, 5, 6, 7, 9, 10a, 10b, 12, 13a, 13b, 13c, 13d, 14, 16a, 20b, 21, 23a, 23b, 23d, 24b, 24c, 27 (one gap) \\
...
\textbf{not done} & \textbf{2, 13e, 17, 19, 20c, 24a, 25, 26} \\
\bottomrule
\end{tabular}
\end{table}

Eight items are \emph{not done}. Four of them --- 2, 24a, 25 and 26 --- are writing.
Three --- 17, 19 and 20a --- are a single script over data already collected. One ---
13e and 20c together --- is a decision about whether to run four pre-specified
subgroups or to state in print that they were pre-specified and not run. None of the
eight requires new screening.
...
7 & Screener S3 submits batch C & 08:01:22 & \textbf{mtime only --- never committed} \\
8 & Screener S1 submits batch A & 08:03:10 & \textbf{mtime only --- never committed} \\
9 & Screener S2 submits batch B & 08:07:06 & \textbf{mtime only --- never committed} \\
10 & Screener S4 submits batch D & 08:07:36 & \textbf{mtime only --- never committed} \\
\bottomrule
\end{tabular}
\end{table}
...
\item \textbf{The four sealed screener files were never committed.} The frozen protocol
says each overlap batch is ``submitted as a sealed file, timestamped in git, before any
screener sees another's codes''. The sealing and the independence held as a matter of
process --- each file carries a written independence statement and no screener's file
references another's --- but \textbf{the git-timestamp part of that requirement was not
met}. Their only timestamps are filesystem mtimes, which are trivially rewritable. The
phrase ``sealed and timestamped in git'' must not be used of these files.
...
\begin{quote}\small
\begin{verbatim}
# the freeze commit, and that nothing later touched protocol, codebook or sample
